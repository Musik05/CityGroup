...
async function getProducts() {
    try {
        ...
        if (!productsData.length) {
            const res = await fetch('./products7.json');
            if (!res.ok) {
		...
		const cardItem = 
			`
                    ...
                    <div class="card" data-product-id="${id}">
                        <div class="card__top">
                            <a href="/card7.html?id=${id}" class="card__image">
                                <img
                                    src="./images/${img}"
                                    alt="${title}"
                                />
                        ...
                        <div class="card__bottom">
                            ...
                            <a href="/card7.html?id=${id}" class="card__title">${title}</a>
                            <div class="card__adress">${adress}</div>
                            <a href="/card7.html?id=${id}"><button class="card__add">Узнать больше</button></a>
                        </div>
                    </div>
            `
